...
/*   Updated: 2024/09/09 12:44:24 by tvan-tui         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_strcapitalize(char *str)
{
	...
	while (str[i] != '\0')
	{
		specialchar = is_specialchar(str[i - 1]);
		if (specialchar && (str[i] >= 97 && str[i] <= 122))
			str[i] = str[i] - 32;
		else if (!specialchar && (str[i] >= 65 && str[i] <= 90))
			str[i] = str[i] + 32;
		i++;
	}
	return (str);
